...
router.get('/search/:title', async (req,res)=>{ 
    const result = await db.query("SELECT * FROM projects WHERE Title LIKE $1", [ '%' + req.params.title + '%'])
...
router.get("/:asin", async (req, res) => {
    try {
        ...
        const project = await db.query(`SELECT * FROM projects WHERE Asin = $1`, [req.params.asin])
...
router.post("/", async (req, res) => {
    try {
        const result = await db.query(`INSERT INTO projects (Asin, Title, Image, Category, Price) 
                                     VALUES ($1,$2,$3,$4,$5) 
                                     RETURNING *`,
...
router.delete("/:asin", async (req, res) => {
    try {
        const result = await db.query(`DELETE FROM projects WHERE asin = $1`, [req.params.asin])
